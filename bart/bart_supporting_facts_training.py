...
logging.basicConfig(filename='qg_incremental_bart_full_context_job.log', filemode='w', level=logging.DEBUG)
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
logging.debug('Using device:' + str(device))

train = pd.read_csv('~/dataset/qg_dataset/hotpot_train_supporting_facts_v1.1.csv')
# train = train[40001:]
valid = pd.read_csv('~/dataset/qg_dataset/hotpot_dev_distractor_supporting_facts_v1.csv')
# valid = valid[:5000]
...
#FIRST TRAINING
config = BartConfig(decoder_start_token_id=tokenizer.pad_token_id)
model = BartForConditionalGeneration(config).from_pretrained(PRETRAINED_MODEL)
model.resize_token_embeddings(len(tokenizer)) # to account for new special tokens
model = model.to(device)
optimizer = torch.optim.SGD(model.parameters(), lr=LR)

SAVED_MODEL_PATH = "/home2/nirmal/dataset/qg_dataset/model_hotpot_supporting_facts.pth"
TEMP_SAVE_PATH = "/home2/nirmal/dataset/qg_dataset/model_hotpot_supporting_facts.pth"
...
torch.save(model, "/home2/nirmal/dataset/qg_dataset/model_hotpot_supporting_facts_last.pth")
md2 = torch.load("/home2/nirmal/dataset/qg_dataset/model_hotpot_supporting_facts_last.pth")
